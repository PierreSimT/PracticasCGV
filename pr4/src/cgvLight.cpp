...
//	set the spotlight parameters: direction, cutoff, exponent. 
	...
	GLfloat ambient[] = { ambientColor[0], ambientColor[1], ambientColor[2], ambientColor[3] };
	GLfloat diffuse[] = { diffuseColor[0], diffuseColor[1], diffuseColor[2], diffuseColor[3] };
	GLfloat specular[] = { specularColor[0], specularColor[1], specularColor[2], specularColor[3] };
	GLfloat position[] = { position[0], position[1], position[2] };
	GLfloat direction[] = { spotlight_direction[0], spotlight_direction[1], spotlight_direction[2] };

	if ( on )
	{
		glLightfv(idLight, GL_POSITION, position);
		glLightfv(idLight, GL_AMBIENT, ambient);
		glLightfv(idLight, GL_DIFFUSE, diffuse);
		glLightfv(idLight, GL_SPECULAR, specular);		
		...
		glLightfv(idLight, GL_SPOT_DIRECTION, direction);
